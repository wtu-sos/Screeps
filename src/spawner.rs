...
pub fn run_spawn(spawn: screeps::objects::StructureSpawn) {
    ...
    debug!("running spawn {}, checking creeps", spawn.name());
    for creep in current_creeps {
        let creep_type = creep.memory().string("type").unwrap_or(Some("harvester".to_string())).unwrap_or("harvester".to_string());
    ...
    if current_fillers.len() < usize::try_from(filler_goal).unwrap() {
        ...
        //info!("running spawn {} ready to build settler!", spawn.name());
        build_settler(spawn);
